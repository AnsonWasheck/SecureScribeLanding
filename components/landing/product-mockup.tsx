...
function ShiftEventItem({ time, type, icon: Icon, color, dot }: (typeof shiftEvents)[0]) {
  return (
    <div className="flex items-center gap-2 rounded-md bg-muted/40 px-2.5 py-2">
      ...
      <span className={`min-w-0 flex-1 truncate text-[11px] font-medium ${color}`}>{type}</span>
      ...
      <div className="col-span-3 flex flex-col gap-3 p-3">
        ...
        <div className="rounded-md border border-border bg-muted/20 p-2.5">
          <div className="mb-1.5 flex items-center gap-1.5">
            ...
            <span className="rounded bg-primary/10 px-1.5 py-px text-[9px] font-medium text-primary">Local capture</span>
            <span className="ml-auto rounded bg-muted px-1.5 py-px text-[9px] text-muted-foreground">09:15</span>
          </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-medium text-muted-foreground">Structured draft</span>
          <div className="h-px flex-1 bg-border" />
        </div>

        {/* Structured fields */}
        <div className="space-y-2 rounded-md border border-primary/15 bg-primary/[0.03] p-2.5">
          {[
            ...
            <div key={field} className="flex gap-2">
              <span className="w-[62px] shrink-0 pt-px text-[9px] font-semibold uppercase tracking-wide text-muted-foreground">
                {field}
              </span>
              <span className={`flex-1 text-[11px] leading-relaxed text-foreground ${italic ? "italic" : ""}`}>
                {value}
                {linked && <Link className="ml-1 inline h-3 w-3 text-primary/60" />}
              </span>
            </div>
          ))}
        </div>
        ...
        {/* Status footer */}
        <div className="mt-auto flex items-center justify-between rounded-md border border-border bg-muted/30 px-2.5 py-2">
          <span className="text-[11px] font-medium text-foreground">Review required before filing</span>
      ...
      <div className="col-span-2 flex flex-col gap-3 p-3">
            ...
            <div key={label} className="flex items-center justify-between rounded bg-muted/40 px-2 py-1.5">
              ...
              <span
                className={`rounded px-1.5 py-px text-[9px] font-medium ${
                  badge === "Pending" ? "bg-amber-100 text-amber-700" : "bg-primary/10 text-primary"
                }`}
              >
                {badge}
              </span>
        ...
        <div>
          <div className="mb-1.5 flex items-center gap-1">
            ...
            <span className="text-[10px] font-semibold text-amber-700">Needs confirmation</span>
          </div>
        ...
        <div className="mt-auto rounded-md border border-primary/20 bg-primary/5 p-2.5">
          <div className="flex items-start gap-1.5">
            <CheckCircle2 className="mt-px h-3.5 w-3.5 shrink-0 text-primary" />
            <p className="text-[11px] leading-relaxed text-foreground">
              Review required before EHR submission
            </p>
          </div>
          <span className="mt-1.5 inline-flex rounded bg-primary/10 px-1.5 py-0.5 text-[9px] font-medium text-primary">
